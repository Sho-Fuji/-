...
public class Jissen7 extends JDialog {
	...
	public Jissen7() {
		...
		ARadioButton = new JRadioButton("先生はいつこちらに参られますか?");
		ARadioButton.setFont(new Font("UD デジタル 教科書体 N-B", Font.PLAIN, 12));
		ARadioButton.setSelected(true);
		panel.add(ARadioButton);
		
		BRadioButton = new JRadioButton("先生はいつこちらにいらっしゃいますか");
				...
				AnswerButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if(ARadioButton.isSelected()) {
							kaisetuLabel.setText("<html>正解です。<br>Aは先生への敬意を表すのに「行く」の謙譲語である「参る」を用いているので誤り。</html>");
							MessageLabel.setText("〇 正解 ");
							MessageLabel.setForeground(Color.RED);
						}
						else if(BRadioButton.isSelected()) {
							kaisetuLabel.setText("<html>不正解です。Aは先生への敬意を表すのに「行く」の謙譲語である「参る」を用いているので誤り。</html>");
